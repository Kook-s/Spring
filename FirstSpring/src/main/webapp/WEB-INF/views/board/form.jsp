...
$(()=>{
	...
	CKEDITOR.config.allowContent = true; //html태그가 사라지는걸 방지
	
	var formBtn = $("#formBtn");
	formBtn.on("click",function(){
		var title = $("#boTitle").val(); //제목 데이터 얻어오기
//		var content = $("#boContent").val();//내용 데이터 얻어오기(이걸로 이제 못가져옴)
		...
		//등록 일때와 수정 일때를 구분해서 방향만 살짝 틀어주면 등록이 되고 수정이 된다.
		if($(this).val()=="수정"){
			$("#boardForm").attr("action","/board/update.do");
		}
		
		//제목, 내용을 누락하지 않았을 경우 최종 submit
		$("#boardForm").submit();
	})
})
</script>
